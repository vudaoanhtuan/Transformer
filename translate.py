...
SRC, TRG = load_field('multi30k.h5')
SCR_NUMWORD = len(SRC.vocab.itos)
TRG_NUMWORD = len(TRG.vocab.itos)

model = get_model(SCR_NUMWORD, TRG_NUMWORD, device=DEVICE)
load_model(model, path="weight/cp_epoch_25.h5")


with open("data/test2016.en", encoding='utf8') as f:
    test = f.read().split('\n')[:-1]

with open("data/tran2016.de", 'w', encoding='utf8', ) as f:
